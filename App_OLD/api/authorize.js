...
module.exports = function(app, db, _) {
        ...
        con.query(`${sqlEstab};${sqlEmpresa}`, function (error, results, fields) {
            ...
            if(_.isEmpty(results[1]) && !_.isEmpty(results[0])) {
                if(typeof body.email !== 'string' && typeof body.password !== 'string')
                  return res.status(400).json("Email and Password require string type");
                if(body.email !== result[0][0].email && !bcrypt.compareSync(body.password, result[0][0].passwordHashed))
                  return res.status(401).send({Auth: "Autenticação falhou, verifique suas credenciais"});
                  try {
            ...
            } else if(!_.isEmpty(results[1]) && _.isEmpty(results[0])) {
              if(typeof body.email !== 'string' && typeof body.password !== 'string')
                return res.status(400).json("Email and Password require string type");
              if(body.email !== result[1][0].email && !bcrypt.compareSync(body.password, result[1][0].passwordHashed))
                return res.status(401).send({Auth: "Autenticação falhou, verifique suas credenciais"});
              try {
                let token = jwt.sign({id: results[1][0].id, tipo: 'empresa'}, 'segredo', {expiresIn: 60*40});
                res.set('x-access-token', token);
                con.release();
                return res.status(200).json({Empresa: results[1][0]});
              } catch(e) {
                return res.status(400).send({Erro: e.toString()});
              }
            }
  ...
  api.verificarTokenEstab = function(req, res, next) {
    var token = req.headers['x-access-token'];
    if(token) {
      jwt.verify(token, 'segredo', function(err, decoded) {
          if(err) {
